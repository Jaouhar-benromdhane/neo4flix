...
$neo4j = docker ps --filter "name=neo4flix-neo4j" --format "{{.Status}}"
if ($neo4j -like "Up*") {
    Write-Host "      Neo4j OK - http://localhost:7474" -ForegroundColor Green
} else {
    Write-Host "      Neo4j ERREUR - verifie Docker Desktop !" -ForegroundColor Red
    exit 1
}
...
Start-Process powershell -ArgumentList "-NoExit", "-Command",
    "cd $ROOT\movie-service; `$env:PATH += ';$MVN'; `$env:JAVA_HOME='E:\java'; mvn spring-boot:run"
Start-Sleep -Seconds 2

# --- 3. user-service ---
Write-Host "[3/7] Demarrage user-service (8082)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command",
    "cd $ROOT\user-service; `$env:PATH += ';$MVN'; `$env:JAVA_HOME='E:\java'; mvn spring-boot:run"
Start-Sleep -Seconds 2

# --- 4. rating-service ---
Write-Host "[4/7] Demarrage rating-service (8083)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command",
    "cd $ROOT\rating-service; `$env:PATH += ';$MVN'; `$env:JAVA_HOME='E:\java'; mvn spring-boot:run"
Start-Sleep -Seconds 2

# --- 5. recommendation-service ---
Write-Host "[5/7] Demarrage recommendation-service (8084)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command",
    "cd $ROOT\recommendation-service; `$env:PATH += ';$MVN'; `$env:JAVA_HOME='E:\java'; mvn spring-boot:run"
Start-Sleep -Seconds 2

# --- 6. api-gateway ---
Write-Host "[6/7] Demarrage api-gateway (8080)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command",
    "cd $ROOT\api-gateway; `$env:PATH += ';$MVN'; `$env:JAVA_HOME='E:\java'; mvn spring-boot:run"
Start-Sleep -Seconds 2
